...
object ScreenTimeMethod {
    ...
    fun blockApps(
        ...
        try {
            ...
            if (!Settings.canDrawOverlays(context)) {
                Log.e("ScreenTimeMethod", "Draw overlay permission not granted")
                return false
            }
    ...
    private fun scheduleWorkManagerUnblock(context: Context, durationMillis: Long) {
        try {
            ...
            // Cancel any existing unblock work
            workManager.cancelUniqueWork(com.solusibejo.screen_time.worker.UnblockWorker.WORK_NAME)
            
            // Create a one-time work request that will execute at the end time
            val unblockWork = OneTimeWorkRequestBuilder<com.solusibejo.screen_time.worker.UnblockWorker>()
            ...
            workManager.enqueueUniqueWork(
                com.solusibejo.screen_time.worker.UnblockWorker.WORK_NAME,
                ExistingWorkPolicy.REPLACE,
                unblockWork
            )
            
            Log.d("ScreenTimeMethod", "Scheduled WorkManager unblock after ${durationMillis}ms")
        } catch (e: Exception) {
            Log.e("ScreenTimeMethod", "Error scheduling WorkManager unblock", e)
        }
    }
    
    /**
     * Cancel WorkManager unblock task
     */
    private fun cancelWorkManagerUnblock(context: Context) {
        try {
            val workManager = WorkManager.getInstance(context)
            workManager.cancelUniqueWork(com.solusibejo.screen_time.worker.UnblockWorker.WORK_NAME)
            Log.d("ScreenTimeMethod", "Cancelled WorkManager unblock task")
        } catch (e: Exception) {
            Log.e("ScreenTimeMethod", "Error cancelling WorkManager unblock", e)
        }
    }
