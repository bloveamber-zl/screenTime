package com.solusibejo.screen_time.const

object Argument {
    const val startTimeInMillisecond = "startTimeInMillisecond"
    const val endTimeInMillisecond = "endTimeInMillisecond"
    const val interval = "interval"
    const val permissionType = "permissionType"
    const val lookbackTimeMs = "lookbackTimeMs"
    const val ignoreSystemApps = "ignoreSystemApps"
    const val packagesName = "packagesName"
    const val duration = "duration"
    const val pauseDuration = "pauseDuration"
    const val pauseEndTime = "pauseEndTime"
    const val layoutPackage = "layoutPackage"
    const val layoutName = "layoutName"
    const val notificationTitle = "notificationTitle"
    const val notificationText = "notificationText"
    const val showNotification = "showNotification"

    // iOS parity arguments
    const val appGroupIdentifier = "appGroupIdentifier"
    const val selectionStorageKey = "selectionStorageKey"
    const val endTime = "endTime"
    const val shieldTitle = "shieldTitle"
    const val shieldSubtitle = "shieldSubtitle"
    const val shieldSubtitleColor = "shieldSubtitleColor"
    const val shieldButtonLabel = "shieldButtonLabel"
    const val shieldButtonColor = "shieldButtonColor"
    const val shieldButtonTextColor = "shieldButtonTextColor"
    const val shieldIconName = "shieldIconName"
    const val shieldIconAsset = "shieldIconAsset"

    const val startHour = "startHour"
    const val startMinute = "startMinute"
    const val endHour = "endHour"
    const val endMinute = "endMinute"

    const val scheduleId = "scheduleId"
    const val startTime = "startTime"
    const val recurring = "recurring"
    const val daysOfWeek = "daysOfWeek"
}